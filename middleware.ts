...
export async function middleware(req: NextRequest) {
  ...
  if (session && !isPublicRoute && !isAuthRoute) {
    ...
    if (error || !userData || !userData.verification) {
    ...
    if (path.startsWith("/admin") && !userData.is_admin) {
      // Redirect non-admin users trying to access admin routes to the dashboard
      return NextResponse.redirect(new URL("/dashboard", req.url))
    }

    // If the user is at the root path, redirect to the appropriate dashboard
    if (path === "/" || path === "") {
      if (userData.is_admin) {
        return NextResponse.redirect(new URL("/admin/attendance-overview", req.url))
      } else {
        return NextResponse.redirect(new URL("/dashboard", req.url))
      }
    }
